...
def sinusoidal_control(t, amplitude, frequency, phase):
    """Generate sinusoidal control input based on time."""
...
def make_der_func(ap, model_str, v2_integrators):
    ...
    def der_func(t, full_state):
        ...
        throttle = sinusoidal_control(t, amplitude=0.2, frequency=0.5, phase=0) + 0.5  # Keeping throttle between 0.3 and 0.7
        elevator = sinusoidal_control(t, amplitude=0.1, frequency=1.0, phase=np.pi/2)
        aileron = sinusoidal_control(t, amplitude=0.1, frequency=1.0, phase=0)
        rudder = sinusoidal_control(t, amplitude=0.05, frequency=1.5, phase=np.pi)
        
        # Combine the control inputs into a single array
        u_refs = np.array([throttle, elevator, aileron, rudder])

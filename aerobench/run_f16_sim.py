...
def run_f16_sim(initial_state, tmax, ap, u_seq, step=1/30, extended_states=False, model_str='morelli',
    ...
    '''
    # if len(u_seq[:, 0]) != int(tmax / step):
    #     raise ValueError("Input length does not match simulation time")
    ...
    if extended_states:
        xd, u, Nz, ps, Ny_r = get_extended_states(ap, times[-1], states[-1], model_str, v2_integrators, u_seq, step)
    ...
    der_func = make_der_func(ap, model_str, v2_integrators, u_seq, step)
...
def make_der_func(ap, model_str, v2_integrators, u_seq, step):
    ...
    def der_func(t, full_state):
        ...
        for i in range(num_aircraft):
            ...
            xd = controlled_f16(t, state, u_ref, ap.llc, u_seq, step, model_str, v2_integrators)[0]
            xds.append(xd)
...
def get_extended_states(ap, t, full_state, model_str, v2_integrators, u_seq, step):
    ...
    for i in range(num_aircraft):
        ...
        xd, u, Nz, ps, Ny_r = controlled_f16(t, state, u_ref, llc, u_seq, step, model_str, v2_integrators)

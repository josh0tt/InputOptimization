...
function run_f16_sim_random_seq(t_horizon::Int64, Δt::Float64, u_seq::Matrix{Float64}, gif=false)
    ### Initial Conditions ###
    power = 9 # engine power level (0-10)
    ...
    def simulate_waypoint(init, alt, gif, step = 1/15):
        'simulate the system, returning waypoints, res'
        ...
        n_pt = init[9]
        h_pt = init[11]
        waypoints = [[e_pt, n_pt + 10000, h_pt + 3000]]
    ...
    """
    filename = "straight_and_level_random_init.gif"
    times, states, controls, Nz = py"simulate"(init, alt, u_seq, round(Int64, t_horizon * Δt), gif, filename, step=Δt)
    ...
    # Waypoint simulation
    new_times, new_states, new_controls, new_Nz = py"simulate_waypoint"(states[end, :], states[end, 12], gif)

    # only keep the first 13 columns of states (the others are integration variables)
    new_states = new_states[:, 1:13]
    for i in 2:9
        new_states[:, i] = rad2deg(new_states[:, i])
    end
    # include Nz in states
    new_states = hcat(new_states, new_Nz)

    # only keep the first 4 columns of controls
    new_controls = new_controls[:, 1:4]
    for i in 2:4
        new_controls[:, i] = rad2deg(new_controls[:, i])
    end

    return vcat(times, times[end] .+ new_times), vcat(states, new_states), vcat(controls, new_controls)
end
